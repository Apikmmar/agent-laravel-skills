# GraphQL Schema — Rules & Conventions
...
- Use `relation:` only when the GraphQL field name differs from the PHP relationship method name
...
## SuccessResponse
All mutations return `SuccessResponse`. It is defined **once in the root schema** (`graphql/schema.graphql`) — never inside a module, never duplicated:
...
## Non-Negotiables
- Never define types inline inside mutation or query files
- Never redefine `SuccessResponse` inside a module
- All enum values UPPERCASE — never lowercase
- All fields snake_case — never camelCase
